    ...
    <div class="el-cascader">
        ...
        >
            ...
            <div class="cascader-menu-wrapper" v-clickoutside="hidePopover">
                <ul v-if="options.length > 0" class="el-cascader-menu cascader-menu" v-for="(cas, index) in casTree"
                    :key="index">
                    <li
                            :class="{
  ...
  export default {
    ...
    methods: {
      ...
      initDatas() {
        ...
        this.recursiveExpandTo();
      },
      /**
       * 显示选项时自动展开到给定的option所属的层级
       */
      recursiveExpandTo() {
        function loop(needle, options) {
          let result = [];
          for (let option of options) {
            if (option.value === needle) {
              ...
              if (option.children && option.children.length > 0) {
                result.push.apply(result, loop(needle, option.children));
                if (result.length > 0) {
        ...
        if (this.expandToLevel) {
          this.$nextTick(_ => {
            if (this.clonedOpts && this.clonedOpts.length > 0) {
              let result = loop(this.expandToLevel, this.clonedOpts);
